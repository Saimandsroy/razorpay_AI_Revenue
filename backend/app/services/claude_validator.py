...
def validate_claude_output(response: str | dict[str, Any] | None) -> tuple[bool, dict[str, Any] | None]:
    """Validate Claude's explanation only; actions are never read from this output."""
    if response is None:
        return False, None
    ...
    if not isinstance(alternatives, list) or any(
        ...
        return False, None
    return True, {"reasoning": response["reasoning"].strip(), "confidence": float(confidence), "alternatives_rejected": alternatives}
